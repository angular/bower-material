 ...
 * v0.8.3-master-74601d0
 */
function(){"use strict";function e(e,t,n,r,o){function a(){d(),o(function(){c(),i()})}function i(){e.autofocus&&C.input.focus()}function d(){var t=parseInt(e.delay,10)||0;e.$watch("searchText",t?n.debounce(l,t):l),e.$watch("selectedItem",function(t,n){t&&(e.searchText=u(t)),e.itemChange&&t!==n&&e.itemChange(f(t))})}function c(){C={main:t[0],ul:t[0].getElementsByTagName("ul")[0],input:t[0].getElementsByTagName("input")[0]}}function l(t,n){if(T.index=p(),(t||t!==n)&&t!==u(e.selectedItem)){if(e.selectedItem=null,!t||t.length<Math.max(parseInt(e.minLength,10),1))return T.loading=!1,T.matches=[],T.hidden=h(),$(),void 0;var r=t.toLowerCase();k&&k.cancel&&(k.cancel(),k=null),!e.noCache&&x[r]?(T.matches=x[r],$()):E(t),T.hidden=h(),e.textChange&&t!==n&&e.textChange(f(e.selectedItem))}}function s(){N||(T.hidden=!0)}function m(e){switch(e.keyCode){case r.KEY_CODE.DOWN_ARROW:if(T.loading)return;e.preventDefault(),T.index=Math.min(T.index+1,T.matches.length-1),A(),M();break;case r.KEY_CODE.UP_ARROW:if(T.loading)return;e.preventDefault(),T.index=Math.max(0,T.index-1),A(),M();break;case r.KEY_CODE.ENTER:if(T.loading||T.index<0)return;e.preventDefault(),b(T.index);break;case r.KEY_CODE.ESCAPE:T.matches=[],T.hidden=!0,T.index=p();break;case r.KEY_CODE.TAB:}}function u(t){return t&&e.itemText?e.itemText(f(t)):t}function f(e){if(e){var t={};return T.itemName&&(t[T.itemName]=e),t}}function p(){return e.autoselect?0:-1}function h(){return 1===T.matches.length&&e.searchText===u(T.matches[0])&&e.selectedItem===T.matches[0]}function g(){return u(T.matches[T.index])}function b(t){e.selectedItem=T.matches[t],e.searchText=u(e.selectedItem)||e.searchText,T.hidden=!0,T.index=0,T.matches=[]}function v(){e.searchText="",b(-1),C.input.focus()}function E(t){function n(n){x[o]=n,t===e.searchText&&(k=null,T.loading=!1,T.matches=n,T.hidden=h(),$())}var r=e.$parent.$eval(y),o=t.toLowerCase();angular.isArray(r)?n(r):(T.loading=!0,r.success&&r.success(n),r.then&&r.then(n),r.error&&r.error(function(){T.loading=!1}))}function $(){if(!T.hidden)switch(T.matches.length){case 0:return T.messages.splice(0);case 1:return T.messages.push({display:"There is 1 match available."});default:return T.messages.push({display:"There are "+T.matches.length+" matches available."})}}function M(){T.messages.push({display:g()})}function A(){var e=41*T.index,t=e+41,n=225.5;e<C.ul.scrollTop?C.ul.scrollTop=e:t>C.ul.scrollTop+n&&(C.ul.scrollTop=t-n)}var T=this,w=e.itemsExpr.split(/ in /i),y=w[1],C=null,k=null,x={},N=!1;return T.scope=e,T.parent=e.$parent,T.itemName=w[0],T.matches=[],T.loading=!1,T.hidden=!0,T.index=null,T.messages=[],T.id=n.nextUid(),T.keydown=m,T.blur=s,T.clear=v,T.select=b,T.getCurrentDisplayValue=g,T.fetch=n.debounce(E),T.listEnter=function(){N=!0},T.listLeave=function(){N=!1},T.mouseUp=function(){C.input.focus()},a()}angular.module("material.components.autocomplete").controller("MdAutocompleteCtrl",e),e.$inject=["$scope","$element","$mdUtil","$mdConstant","$timeout"]}(),/*!
 * Angular Material Design
 * https://github.com/angular/material
 * @license MIT
 * v0.8.3-master-74601d0
