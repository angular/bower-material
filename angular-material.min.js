 ...
 * v0.6.1-master-8abebce
 */
function(){"use strict";function e(e,t,n,r,o){function a(a,i){var c=a.find("md-tab-label");c.length?c.remove():c=angular.isDefined(i.label)?angular.element("<md-tab-label>").html(i.label):angular.element("<md-tab-label>").append(a.contents().remove());var d=a.contents().remove();return function(a,i,l,m){function u(){var e=c.clone();i.append(e),t(e)(a.$parent),v.content=d.clone()}function s(){a.$apply(function(){$.select(v),$.focus(v)})}function f(e){e.keyCode==r.KEY_CODE.SPACE||e.keyCode==r.KEY_CODE.ENTER?(i.triggerHandler("click"),e.preventDefault()):e.keyCode===r.KEY_CODE.LEFT_ARROW?a.$evalAsync(function(){$.focus($.previous(v))}):e.keyCode===r.KEY_CODE.RIGHT_ARROW&&a.$evalAsync(function(){$.focus($.next(v))})}function p(e){a.$apply(function(){"swipeleft"===e.type?$.select($.next()):$.select($.previous())})}function h(){a.$watch("$parent.$index",function(e){$.move(v,e)})}function g(){function e(e){var t=$.getSelectedItem()===v;e&&!t?$.select(v):!e&&t&&$.deselect(v)}var t=a.$parent.$watch("!!("+l.mdActive+")",e);a.$on("$destroy",t)}function b(){function e(e){i.attr("aria-disabled",e);var t=$.getSelectedItem()===v;t&&e&&$.select($.next()||$.previous())}a.$watch(v.isDisabled,e)}function E(){var e=l.id||"tab_"+n.nextUid();if(i.attr({id:e,role:"tab",tabIndex:-1}),d.length){var t="content_"+e;i.attr("aria-controls")||i.attr("aria-controls",t),v.contentContainer.attr({id:t,role:"tabpanel","aria-labelledby":e})}}var v=m[0],$=m[1];a.$watch(function(){return l.label},function(){o(function(){$.scope.$broadcast("$mdTabsChanged")},0,!1)}),u(),E();var M=e.attachTabBehavior(a,i,{colorElement:$.inkBarElement});$.add(v),a.$on("$destroy",function(){M(),$.remove(v)}),angular.isDefined(l.ngClick)||i.on("click",s),i.on("keydown",f),a.onSwipe=p,angular.isNumber(a.$parent.$index)&&h(),angular.isDefined(l.mdActive)&&g(),b()}}return{restrict:"E",require:["mdTab","^mdTabs"],controller:"$mdTab",scope:{onSelect:"&mdOnSelect",onDeselect:"&mdOnDeselect",label:"@"},compile:a}}angular.module("material.components.tabs").directive("mdTab",e),e.$inject=["$mdInkRipple","$compile","$mdUtil","$mdConstant","$timeout"]}(),/*!
 * Angular Material Design
 * https://github.com/angular/material
 * @license MIT
 * v0.6.1-master-8abebce
